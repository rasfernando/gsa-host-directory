    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-8 space-y-4">
        {(trips ?? []).map((t) => {
          ...
          // Balance: what's still owed after the deposit credit and payments.
          ...
          if (plan) {
            ...
            if (upcoming)
              nextDue = { amount: upcoming.amount_pennies, date: upcoming.due_date };
          }
          ...
          return (
            <Link
              key={t.id}
              href={`/trips/${t.id}`}
              className="block rounded-2xl border border-stone-200/70 bg-white p-6 shadow-sm transition-all duration-150 hover:-translate-y-0.5 hover:border-stone-300 hover:shadow-md"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-base font-semibold text-stone-900">
                    {host?.name ?? t.country ?? "New trip"}
                  </p>
              ...
              <div className="mt-4 flex flex-wrap gap-x-8 gap-y-2 text-sm">
                {subtotal > 0 && (
                  <span className="text-stone-600">
                    Basket{" "}
                    <strong className="text-stone-900">{formatPounds(subtotal)}</strong>
                  </span>
                )}
                {balance != null && (
                  <span className="text-stone-600">
                    ...
                    <strong className="text-stone-900">
                      {formatPounds(Math.max(balance, 0))}
                    </strong>
                  </span>
                )}
                {nextDue && (
        ...
        {(trips ?? []).length === 0 && (
          <div className="rounded-2xl border border-stone-200/70 bg-white p-10 text-center shadow-sm">
